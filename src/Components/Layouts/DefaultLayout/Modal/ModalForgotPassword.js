...
import { forgotPasswordService } from "../../../Services/AccountServices/ForgotPasswordService";
...
function ModalForgotPassword({
  ...
  const onSubmit = async (data) => {
    ...
    if (res.status === 200) {
      setLoading(false);
      reset();
      setExpiredTime(res.data.expiration);
      setOpen(false);
        ...
        >
          ...
          >
            {loading ? (
              <svg
                aria-hidden="true"
                class="inline w-6 h-6 text-gray-200 animate-spin dark:text-gray-600 fill-white"
                viewBox="0 0 100 101"
                fill="none"
